(() => {
  ...
  function isElementVisible(element,win=globalThis.window){
    ...
    const doc=element.ownerDocument||win?.document||globalThis.document;
  ...
  function classifySendAttempt({busy,composerText,queuedText,sendReady,acceptBusy=true}){if(busy&&acceptBusy)return'accepted';const current=normalizeComparableText(composerText);if(!current&&!sendReady)return'accepted';if(current&&!composerTextMatchesQueued(current,queuedText))return'interrupted';return'pending';}
  ...
  function composerTextMatchesQueued(composerText,queuedText){
    const current=normalizeComparableText(composerText);
    const queued=normalizeComparableText(queuedText);
  ...
  function classifyPreparedSendState({active=true,composerMatches=true,sendReady=false}={}){
    ...
    if(!sendReady)return'waiting';
    if(composerMatches)return'ready';
    return'waiting';
  }
  function requestComposerSubmit(composer,sendButton){
    ...
    try{form.requestSubmit(sendButton);return true;}catch{
      try{form.requestSubmit();return true;}catch{return false;}
    }
  }

  function dispatchEnterKey(composer,win=globalThis.window){
    if(!composer?.dispatchEvent)return false;
    composer.focus?.();
    ...
    composer.dispatchEvent(new KeyboardEventCtor('keydown',options));
    composer.dispatchEvent(new KeyboardEventCtor('keyup',options));
    return true;
  }
  ...
  function setComposerText(composer,text){
    ...
    const doc=composer.ownerDocument||globalThis.document;
