(() => {
  ...
  const SEND_SELECTORS = ['#composer-submit-button[data-testid="send-button"]','button[data-testid="send-button"]','[role="button"][data-testid="send-button"]','button#composer-submit-button:not([data-testid="stop-button"])'];
  const STOP_SELECTORS = ['#composer-submit-button[data-testid="stop-button"]','button[data-testid="stop-button"]','button[aria-label*="Stop generating" i]','button[aria-label*="Stop streaming" i]'];
  ...
  const BATCH_MUTATION_MESSAGE = 'aichat:chatgpt-batch-mutate';
  ...
  function findSendButton(composer,doc=globalThis.document,win=globalThis.window) { const local=dom.firstVisible(composerScope(composer)||doc,SEND_SELECTORS,win); return local||dom.firstVisible(doc,SEND_SELECTORS,win); }
  ...
  function normalizeBatchIds(ids){const result=[];const seen=new Set();for(const value of Array.from(ids||[])){const id=String(value||'').trim();if(!id||seen.has(id))continue;seen.add(id);result.push(id);}return result;}
  function sendRuntimeMessage(runtime,message){return new Promise((resolve,reject)=>{let settled=false;const finish=(fn,value)=>{if(settled)return;settled=true;fn(value);};try{const pending=runtime.sendMessage(message,(response)=>{const runtimeError=runtime.lastError;if(runtimeError)finish(reject,new Error(runtimeError.message||String(runtimeError)));else finish(resolve,response);});if(pending&&typeof pending.then==='function')pending.then((response)=>finish(resolve,response),(error)=>finish(reject,error));}catch(error){finish(reject,error);}});}
  function normalizeBatchResult(response,ids){if(!Array.isArray(response?.succeeded)||!Array.isArray(response?.failed))throw new Error('Invalid ChatGPT batch response');const requested=normalizeBatchIds(ids);const succeededSet=new Set(response.succeeded.map((id)=>String(id)));const failedMap=new Map(response.failed.map((entry)=>[String(entry?.id||''),entry]));const succeeded=[];const failed=[];for(const id of requested){if(succeededSet.has(id)){succeeded.push(id);continue;}const entry=failedMap.get(id);failed.push({id,error:String(entry?.error||'ChatGPT batch returned no result')});}return {succeeded,failed};}
  async function runBatchAction(action,ids,context={}){const list=normalizeBatchIds(ids);if(!list.length)return {succeeded:[],failed:[]};if(action!=='archive'&&action!=='delete')throw new Error(`Unsupported ChatGPT batch action: ${action}`);const runtime=context.runtime||context.window?.chrome?.runtime||globalThis.chrome?.runtime;if(!runtime?.sendMessage)throw new Error('ChatGPT batch runtime unavailable');const authHeaders=await resolveBatchAuthHeaders(context);const message={type:BATCH_MUTATION_MESSAGE,action,ids:list};if(authHeaders?.authorization)message.auth={authorization:authHeaders.authorization,chatgptAccountId:authHeaders['chatgpt-account-id']||''};const response=await sendRuntimeMessage(runtime,message);return normalizeBatchResult(response,list);}
  const batch={supportsArchive:true,findConversationSection,findConversationHeader,listConversationRows,getConversationId,getConversationAnchor,getNativeButtonTemplate,resolveBatchAuthHeaders,archiveConversation,deleteConversation,runBatchAction};
