...
test('shortcut mode sends only when idle and queue empty',()=>{assert.equal(controller.shortcutMode({busy:false,queueLength:0}),'send');assert.equal(controller.shortcutMode({busy:true,queueLength:0}),'queue');assert.equal(controller.shortcutMode({busy:false,queueLength:1}),'queue');});
...
test('paused state is included in dispatch gate decision helper',()=>{assert.equal(controller.canAutoDispatch({paused:true,busy:false,sendReady:true,queueLength:1,dispatching:false,awaitingBusy:false}),false);assert.equal(controller.canAutoDispatch({paused:false,busy:false,sendReady:true,queueLength:1,dispatching:false,awaitingBusy:false}),true);});
...
test('handleQueueShortcut performs native send for idle empty queue', async () => {
  const state = new controller.ActiveQueueState();
  state.switchTo('chatgpt:conversation:a', { paused: false, items: [] });
  const calls = [];
  const send = { isConnected:true, disabled:false, getBoundingClientRect(){ return {width:10,height:10}; }, getAttribute(){return null;}, click(){calls.push('send');} };
  const composer = { text:'hello' };
  const provider = {
  ...
  const result = await controller.handleQueueShortcut({ provider, state, composer, event, persist:async()=>calls.push('persist'), win:{getComputedStyle:()=>({display:'block',visibility:'visible',pointerEvents:'auto'})} });
  assert.equal(result, 'sent');
  assert.deepEqual(calls, ['prevent','stop','send']);
...
test('DispatchGate sends exactly one item across a busy then idle response cycle', () => {
  ...
  const ready = { paused:false, busy:false, sendReady:true, queueLength:2 };
...
test('DispatchGate releases a failed dispatch so the same queue item can retry', () => {
  const gate = new controller.DispatchGate();
  const ready = { paused:false, busy:false, sendReady:true, queueLength:1 };
